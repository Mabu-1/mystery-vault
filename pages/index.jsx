          ...
          {loading ? (
            ...
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '600px' }}>
                ...
                {/* ENVELOPES TAB */}
                {tab === 'envelopes' && <>
                  <thead><tr><Th>Envelope</Th><Th>Status</Th><Th>Customer Email</Th><Th>Order ID</Th><Th>Claimed At</Th></tr></thead>
                  <tbody>
                    {filteredEnvelopes.map((env, i) => (
                      ...
                      >
                        <td style={{ padding: '14px 20px', fontWeight: 700, color: '#3b82f6' }}>#{env.envelope_number}</td>
                        <Td><span style={{
                          padding: '4px 10px', borderRadius: '20px', fontSize: '12px', fontWeight: 700,
                          background: env.status === 'claimed' ? '#dcfce7' : '#f1f5f9',
                          color: env.status === 'claimed' ? '#16a34a' : '#94a3b8',
                        }}>{env.status === 'claimed' ? '✅ Claimed' : 'Unclaimed'}</span></Td>
                    ...
                    {filteredCustomers.length === 0 ? (
                      ...
                      return (
                        <tr key={i}
                          ...
                          onMouseLeave={e => e.currentTarget.style.background = i % 2 === 0 ? '#fff' : '#fafafa'}
                        >
                          <td style={{ padding: '14px 20px', fontWeight: 600, color: '#1e293b' }}>{c.email}</td>
                          <Td>{c.order_id || '—'}</Td>
                          ...
                          }}>{claimed ? '✅ Claimed' : '⏳ Pending'}</span></Td>
                  ...
                  <tbody>
                    {filteredShipping.length === 0 ? (
                      ...
                      <tr key={i}
                        ...
                        onMouseLeave={e => e.currentTarget.style.background = i % 2 === 0 ? '#fff' : '#fafafa'}
                      >
                        <td style={{ padding: '14px 20px', fontWeight: 600, color: '#1e293b' }}>{s.email}</td>
                        <Td>{s.full_name || '—'}</Td>
